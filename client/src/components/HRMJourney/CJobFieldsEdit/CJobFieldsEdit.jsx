...
function CJobFieldsEdit(props) {
    ...
    const [jobStatus, setJobStatus] = useState(""); // New state for job status
    ...
    const handleEnableJob = async () => {
        ...
        const headers = { Authorization: `${localStorage.getItem("token")}` };
        try {
            const response = await axios.put(`/api/job/enableJob/${job._id}`, job, { headers });
            console.log("Job Enabled!", response.data);
            toast.success('Job Enabled');
            // setJobStatus("Active"); // Update job status
        } catch (err) {
            console.error("Error enabling the job:", err); // Log any errors
        } finally {
        ...
        const getJob = async () => {
            await axios
                ...
                .then((res) => {
                    ...
                    setJobStatus(res.data.job.jobStatus);
                })
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="topBar">
                                ...
                                <div className="buttonContainer">
                                    ...
                                    {jobStatus === "Inactive" ? (
                                        <button className="preview" style={{ backgroundColor: 'gray', color: 'white' }} disabled>
                                            Job Disabled
                                        </button>
                                    ) : (
                                        <button type="submit" className="preview" onClick={handleSubmit}>
